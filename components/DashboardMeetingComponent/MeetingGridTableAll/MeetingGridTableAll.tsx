...
const MeetingGridTableAllMemo: FC<Props> = ({ title }) => {
  ...
  console.log(
    "🔥MeetingGridTableAllMemoコンポーネント 入れ替え後のカラム editedColumnHeaderItemList ",
  ...
  const isUpdateRequiredForLatestSelectedRowDataMeeting = useDashboardStore(
    (state) => state.isUpdateRequiredForLatestSelectedRowDataMeeting
  );
  ...
  // const [meetingColumnHeaderItemList, setMeetingColumnHeaderItemList] = useState<ColumnHeaderItemList[]>([]);
  ...
  const columnIndexToHeaderObjMap = useMemo(() => {
    return new Map(meetingColumnHeaderItemList.map((obj) => [obj.columnIndex, obj]));
  ...
  useEffect(() => {
    ...
    setMeetingColumnHeaderItemList([...editedColumnHeaderItemList]);
    // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ================
    const meetingColumnHeaderItemListJSON = JSON.stringify(editedColumnHeaderItemList);
  ...
  // ================= 🔥🔥テスト🔥🔥==================
  // 新規サーチで検索した条件のnewSearchMeeting_Contact_CompanyParamsのオブジェクトを全てa-zで並べ替えた状態で文字列にすることで、
  ...
  const { status, data, error, isFetching, isFetchingNextPage, fetchNextPage, hasNextPage, refetch } = useInfiniteQuery(
    {
      ...
      queryFn: async (ctx) => {
        ...
        // 新規サーチなしの通常モード
        if (newSearchMeeting_Contact_CompanyParams === null) {
          console.log("通常フェッチ queryFn✅✅✅", newSearchMeeting_Contact_CompanyParams);
          return fetchServerPage(50, ctx.pageParam); // 50個ずつ取得
        } else {
          console.log("サーチフェッチ queryFn✅✅✅", newSearchMeeting_Contact_CompanyParams);
  ...
  useEffect(() => {
    ...
    if (localStorageColumnHeaderItemListJSON) {
      ...
      setMeetingColumnHeaderItemList(localStorageColumnHeaderItemList);
    ...
    const tempFirstColumnItemListArray = meetingColumnHeaderItemList.map((item) => item.columnName);
    ...
    setMeetingColumnHeaderItemList(firstColumnItemListArray);
    // isFrozenがtrueの個数をRefに格納
  ...
  const handleMouseDown = (e: React.MouseEvent, index: number) => {
    ...
    const handleMouseUp = () => {
      ...
      // ================ meetingColumnHeaderItemListも合わせてサイズを更新 テスト ================
      let newColumnHeaderItemList: any[] = [];
      const copyColumnHeaderItemList = [...meetingColumnHeaderItemList];
      ...
      setMeetingColumnHeaderItemList(newColumnHeaderItemList);
      // ================ meetingColumnHeaderItemListも合わせてサイズを更新 テスト ================
  ...
  const columnHeaderLeft = (index: number) => {
    ...
    // isFrozenがtrueなら
    if (meetingColumnHeaderItemList[index].isFrozen) {
  ...
  useEffect(() => {
    ...
    const draggingElementColIndex = meetingColumnHeaderItemList[dragColumnIndex].columnIndex;
    const dropElementColIndex = meetingColumnHeaderItemList[dropIndex!].columnIndex;
    const draggingElementColWidth = meetingColumnHeaderItemList[dragColumnIndex].columnWidth;
    const dropElementColWidth = meetingColumnHeaderItemList[dropIndex!].columnWidth;
    ...
    console.log("🌟更新前 meetingColumnHeaderItemList全体", meetingColumnHeaderItemList);
    //  🌟順番を入れ替える meetingColumnHeaderItemList
    const copyListItems: ColumnHeaderItemList[] = JSON.parse(JSON.stringify(meetingColumnHeaderItemList)); // 一意性を守るため新たなカラムリストを生成
    ...
    // ================= 🔥🔥テスト🔥🔥==================
    setMeetingColumnHeaderItemList([...newListItemArray]);
  ...
  const handleFrozen = (index: number) => {
    ...
    // 🔥フローズンを付与するルート =================================
    if (meetingColumnHeaderItemList[index].isFrozen === false) {
      console.log("🔥フローズンを付与するルート ============================");
      // ✅順番を入れ替え処理 一意性を守るため新たなカラムリストを生成
      const copyColumnHeaderListItems: ColumnHeaderItemList[] = JSON.parse(JSON.stringify(meetingColumnHeaderItemList));
  ...
  // ======= 🌟面談編集モーダルでUPDATE後に選択中のセルをクリックさせてselectedRowDataPropertyを最新に更新する🌟
  ...
  useEffect(() => {
    if (!isUpdateRequiredForLatestSelectedRowDataMeeting) return;
    if (!selectedGridCellRef.current) return;

    console.log("面談UPDATE検知🔥 選択セルクリックで下メインコンテナを最新状態に反映", selectedGridCellRef.current);
    selectedGridCellRef.current.click(); // セルクリック

    setIsUpdateRequiredForLatestSelectedRowDataMeeting(false);
  }, [isUpdateRequiredForLatestSelectedRowDataMeeting, setIsUpdateRequiredForLatestSelectedRowDataMeeting]);
  // ======= ✅面談編集モーダルでUPDATE後に選択中のセルをクリックさせてselectedRowDataPropertyを最新に更新する✅

  // 🌟現在のカラム.map((obj) => Object.values(row)[obj.columnId])で展開してGridセルを表示する
  // カラムNameの値のみ配列バージョンで順番入れ替え
  // ================= 🔥🔥テスト🔥🔥==================
  const columnOrder = [...meetingColumnHeaderItemList].map((item, index) => item.columnName as keyof Client_company); // columnNameのみの配列を取得
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  ...
                  handleOpenTooltip({
                    ...
                    // content: `${
                    //   isColumnHeader && Number(activeCell?.ariaColIndex) !== 1
                    //     ? `カラムを固定`
                    //     : `カラムヘッダーを選択することで、`
                    // }`,
                    // content2: `${
                    //   isColumnHeader && Number(activeCell?.ariaColIndex)
                    //     ? ``
                    //     : `左右スクロール時にカラムを左端に固定できます`
                    // }`,
          ...
          >
            ...
            >
...

              {
                ...
                // !!meetingColumnHeaderItemList.length &&
                //   meetingColumnHeaderItemList
                !!meetingColumnHeaderItemList.length &&
                  [...meetingColumnHeaderItemList]
                    ...
                    .map((key, index) => (
                      <div
                        ...
                        data-text={`${columnNameToJapaneseMeeting(key.columnName)}`}
              ...
              <>
                ...
                >
                  {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    const isAlert =
                      (rowData as Meeting_row_data)?.call_careful_flag || !!(rowData as Meeting_row_data)?.claim;
                      ...
                      >
                        ...
                        >
                          <div className={styles.grid_select_cell_header}>
                            <input
                              ...
                              onChange={(e) => {
                                if (typeof rowData?.meeting_id === "undefined") return;
                                if (rowData?.meeting_id === null) return;
                                console.log(
                                  `クリック VirtualRow.index: ${virtualRow.index} row.meeting_id${rowData.meeting_id}`
                                );
                        ...
                        {rowData ? (
                          ...
                          columnOrder ? (
                              ...
                              .map((value, index) => {
                                ...
                                return (
                                  <div
                                    ...
                                    role="gridcell"
                                    aria-colindex={
                                      meetingColumnHeaderItemList[index]
                                        ? meetingColumnHeaderItemList[index]?.columnIndex
                                        : index + 2
                                    } // カラムヘッダーの列StateのcolumnIndexと一致させる
                                    aria-selected={false}
                                    tabIndex={-1}
                                    className={`${styles.grid_cell} ${
                                      meetingColumnHeaderItemList[index].isFrozen ? styles.grid_column_frozen : ""
                                    } ${
                                      isFrozenCountRef.current === 1 && index === 0 ? styles.grid_cell_frozen_last : ""
                                    } ${isFrozenCountRef.current === index + 1 ? styles.grid_cell_frozen_last : ""}  ${
                                    ...
                                    style={
                                      meetingColumnHeaderItemList[index].isFrozen
                                        ? {
                                            gridColumnStart: meetingColumnHeaderItemList[index]
                                              ? meetingColumnHeaderItemList[index]?.columnIndex
                                              : index + 2,
                            ...
                            //   >
                            //     {/* {value} */}
                            //     {meetingColumnHeaderItemList[index].columnName === "meeting_date" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd")}
                            //     {meetingColumnHeaderItemList[index].columnName === "scheduled_follow_up_date" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd")}
                            //     {meetingColumnHeaderItemList[index].columnName === "meeting_created_at" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd HH:mm:ss")}
                            //     {meetingColumnHeaderItemList[index].columnName === "meeting_updated_at" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd HH:mm:ss")}
                            //     {meetingColumnHeaderItemList[index].columnName !== "scheduled_follow_up_date" &&
                            //       meetingColumnHeaderItemList[index].columnName !== "meeting_date" &&
...
export const MeetingGridTableAll = memo(MeetingGridTableAllMemo);
