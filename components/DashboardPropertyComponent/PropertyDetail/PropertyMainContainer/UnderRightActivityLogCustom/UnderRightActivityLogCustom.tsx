...
const UnderRightActivityLogCustomMemo: FC = () => {
  ...
  const debouncedEnableFetch = useRef(
    debounce((rowDataId) => {
      if (!rowDataId) return console.log("右下 debounceコールバック関数 selectedData無しのためリターン");
  ...
  if (selectedData?.contact_id !== currentRowDataContactId) {
  ...
  useEffect(() => {
    console.log("右下 selectedDataの切り替え検知🌟 debouncedEnableFetch実行");
    setIsFetchingEnabled(false); // データフェッチを無効化
    fetchEnabledRef.current = false;
    debouncedEnableFetch(selectedData?.contact_id); // デバウンス処理 3秒後にisFetchingEnabledをtrueに変更
  ...
  if (!userProfileState?.company_id || !selectedData?.contact_id) {
  ...
  if (!!userProfileState?.company_id && !!selectedData && !!selectedData?.contact_id) {
    fetchServerPage = async (
      limit: number,
      offset: number = 0
    ): Promise<{ rows: TableDataType[] | null; nextOffset: number; isLastPage: boolean; count: number | null }> => {
      ...
      try {
        ...
        const selectPayload = {
          ...
          _contact_id: selectedData.contact_id,
        };
        const {
          data,
          error,
  ...
  const {
    ...
    queryKey: ["under_right_activities_contacts", `${!!selectedData?.contact_id ? selectedData.contact_id : null}`],
    queryFn: async (ctx) => {
    ...
    <>
      ...
      >
        ...
        <div className={`${styles.right_table_tab_header}`}>
          ...
          <div
            className={`flex-center transition-bg03 group ml-[22px] space-x-[9px] px-[10px] py-[2px] ${
              !!selectedData && !isLoading && status === "success" && !refetchLoading
            ...
            onClick={async () => {
              if (!selectedData?.contact_id) return;
              fetchEnabledRef.current = true;
              setRefetchLoading(true);
              await queryClient.invalidateQueries({
                queryKey: ["under_right_activities_contacts", `${selectedData.contact_id}`],
              });
        ...
        >
          ...
          {!(allRows.length > 0) && !!selectedData && isLoading && (
        ...
        <UnderRightGridTableFooter
          ...
          getTotalCount={isLoading ? null : data ? data.pages[0].count : 0}
        />
        {/* ================== Gridフッター ここまで ================== */}
      </div>
    </>
...
export const UnderRightActivityLogCustom = memo(UnderRightActivityLogCustomMemo);
