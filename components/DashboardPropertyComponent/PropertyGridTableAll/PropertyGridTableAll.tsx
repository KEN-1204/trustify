...
import { convertObjToText, searchObjectColumnsSetProperty } from "@/utils/Helpers/MainContainer/commonHelper";
...
const PropertyGridTableAllMemo: FC<Props> = ({ title }) => {
  ...
  const columnIndexToHeaderObjMap = useMemo(() => {
    return new Map(propertyColumnHeaderItemList.map((obj) => [obj.columnIndex, obj]));
  ...
  const searchParamsString = useMemo(() => {
    if (newSearchProperty_Contact_CompanyParams) {
      return Object.entries(newSearchProperty_Contact_CompanyParams)
        .sort(([keyA], [keyB]) => keyA.localeCompare(keyB))
        .map(([key, value]) =>
          searchObjectColumnsSetProperty.has(key)
  ...
  return (
    <>
        ...
        <div className={`${styles.grid_main_container}`}>
          ...
          <div className={`${styles.grid_function_header}`}>
            ...
            <div className={`flex max-h-[26px] w-full min-w-max items-center justify-end space-x-[6px]`}>
              ...
              {activeCell?.role !== "columnheader" && selectedRowDataProperty && (
                <>
                  {!isLoadingDelete && (
                    <button
              ...
              <>
                ...
                >
                  {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    const isAlert =
                      (rowData as Property_row_data)?.call_careful_flag || !!(rowData as Property_row_data)?.claim;
      ...
      {isOpenDeleteModal &&
        selectedRowDataProperty &&
        userProfileState &&
        selectedRowDataProperty.property_created_by_company_id === userProfileState?.company_id && (
          <ConfirmationModal
            titleText={`削除してもよろしいですか？`}
            sectionP1={`確定することでこの案件データは完全に削除されます。\nこの操作は確定後、取り消すことができません。`}
            cancelText="戻る"
            submitText="削除を確定"
            buttonColor="red"
            zIndex="3000px"
            ...
            clickEventSubmit={async () => {
              ...
              try {
                ...
                console.log("🔥削除実行 propertyId", propertyId, "selectedRowDataProperty", selectedRowDataProperty);
