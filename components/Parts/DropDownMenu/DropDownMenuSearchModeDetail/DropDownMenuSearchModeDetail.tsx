...
export const DropDownMenuSearchModeDetail = ({ setIsOpenDropdownMenuSearchMode, language = "ja" }: Props) => {
  ...
  const handleUpdateSubmit = () => {
    ...
    // setIsFetchAllDepartments(isAllDepartmentLocal);
    // setIsFetchAllUnits(isAllUnitLocal);
    // setIsFetchAllOffices(isAllOfficeLocal);
    // setIsFetchAllMembers(isAllMemberLocal);

    setIsOpenDropdownMenuSearchMode(false);
  };

  const detailArray = [
    { title_ja: "事業部", title_en: "Department", state: isAllDepartmentLocal, dispatch: setIsAllDepartmentLocal },
    { title_ja: "係・チーム", title_en: "Unit", state: isAllUnitLocal, dispatch: setIsAllUnitLocal },
    { title_ja: "事業所・営業所", title_en: "Office", state: isAllOfficeLocal, dispatch: setIsAllOfficeLocal },
    { title_ja: "メンバー", title_en: "Member", state: isAllMemberLocal, dispatch: setIsAllMemberLocal },
  ];
      ...
      >
        ...
        <div className={`relative flex h-auto w-full flex-col items-start space-y-[6px] px-[24px] pt-[16px]`}>
          ...
          <p className="text-start text-[12px] text-[var(--color-text-title)]">
            「事業部」「係・チーム」「事業所・営業所」「メンバー」の各項目で検索条件の切り替えが可能です。
          </p>
        </div>
                  ...
                  <div>
                    ...
                    {!isAllMemberLocal && obj.title_ja === "メンバー" && (
                      <select
                        ...
                        onChange={(e) => {
                          if (obj.title_ja === "メンバー" && e.target.value === "All") {
                      ...
                      >
                        {language === "ja" && (
                          <>
                            ...
                            {obj.title_ja === "メンバー" && <option value="Own">{`自身のデータ`}</option>}
                          </>
                        )}
                        {language === "en" && (
                          <>
                            <option value="All">{`All ${obj.title_en}`}</option>
                            {obj.title_ja === "メンバー" && <option value="Own">{`My data`}</option>}
                          </>
                        )}
                      </select>
                    )}
                    {isAllMemberLocal && (
                      <select
                        ...
                        value={obj.state ? `All` : `Own`}
                        onChange={(e) => {
                          if (obj.title_ja === "メンバー" && e.target.value === "Own") {
                      ...
                      >
                        {language === "ja" && (
                          <>
                            <option value="All">{`全${obj.title_ja}`}</option>
                            {obj.title_ja === "事業部" && <option value="Own">{`自${obj.title_ja}`}</option>}
                            {obj.title_ja === "係・チーム" && !isAllDepartmentLocal && (
                              <option value="Own">{`自${obj.title_ja}`}</option>
                            )}
                            {obj.title_ja === "事業所・営業所" && <option value="Own">{`自${obj.title_ja}`}</option>}
                            {obj.title_ja === "メンバー" && <option value="Own">{`自身のデータ`}</option>}
                          </>
                        )}
                        {language === "en" && (
                          <>
                            <option value="All">{`All ${obj.title_en}`}</option>
                            {obj.title_ja === "事業部" && <option value="Own">{`Own ${obj.title_en}`}</option>}
                            {obj.title_ja === "係・チーム" && !isAllDepartmentLocal && (
                              <option value="Own">{`Own ${obj.title_en}`}</option>
                            )}
                            {obj.title_ja === "事業所・営業所" && <option value="Own">{`Own ${obj.title_en}`}</option>}
                            {obj.title_ja === "メンバー" && <option value="Own">{`My data`}</option>}
                          </>
                        )}
                      </select>
