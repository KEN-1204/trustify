...
import { Feature3TextEn, Feature3TextJa, Feature5TextEn, Feature5TextJa } from "./data";
...
const FeatureParagraph5Memo: FC<Props> = ({ hoveredFeature, featureSection }) => {
  const language = useStore((state) => state.language);
  const startAnimationFeature5 = useStore((state) => state.startAnimationFeature5);
  // タイプライターアニメーションの実装
  ...
  const textMaterial = language === "ja" ? Feature5TextJa : Feature5TextEn;

  useEffect(() => {
    if (!startAnimationFeature5) return;
    if (typeof textMaterial === "undefined") return;
    if (running) return console.log("5リターン！");
    if (done) return;
    ...
    const typeWriterEvent = () => {
      ...
      console.log("タイプライタースタート5", "indexとnewIndex", index, newIndex, "代入後newText", newText);
      setTimeout(
        () => {
          setText(newText);
          setIndex(newIndex);
    ...
    typeWriterEvent();
  }, [text, index, startAnimationFeature5]);
...
export const FeatureParagraph5 = memo(FeatureParagraph5Memo);
