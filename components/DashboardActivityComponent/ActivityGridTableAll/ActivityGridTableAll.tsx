...
import { convertObjToText, searchObjectColumnsSetActivity } from "@/utils/Helpers/MainContainer/commonHelper";
...
const ActivityGridTableAllMemo: FC<Props> = ({ title }) => {
  ...
  // const [activityColumnHeaderItemList, setactivityColumnHeaderItemList] = useState<ColumnHeaderItemList[]>([]);
  ...
  // ================= 🔥🔥テスト🔥🔥==================
  // 新規サーチで検索した条件のnewSearchActivity_Contact_CompanyParamsのオブジェクトを全てa-zで並べ替えた状態で文字列にすることで、
  ...
  const searchParamsString = useMemo(() => {
    if (newSearchActivity_Contact_CompanyParams) {
      return Object.entries(newSearchActivity_Contact_CompanyParams)
        .sort(([keyA], [keyB]) => keyA.localeCompare(keyB))
        .map(([key, value]) =>
          searchObjectColumnsSetActivity.has(key)
  ...
  const { status, data, error, isFetching, isFetchingNextPage, fetchNextPage, hasNextPage, refetch } = useInfiniteQuery(
    {
      ...
      queryFn: async (ctx) => {
        ...
        // 新規サーチなしの通常モード
        if (newSearchActivity_Contact_CompanyParams === null) {
          console.log("通常フェッチ queryFn✅✅✅", newSearchActivity_Contact_CompanyParams);
          return fetchServerPage(50, ctx.pageParam); // 50個ずつ取得
        } else {
          console.log("サーチフェッチ queryFn✅✅✅", newSearchActivity_Contact_CompanyParams);
  ...
  useEffect(() => {
    ...
    if (localStorageColumnHeaderItemListJSON) {
      ...
      setActivityColumnHeaderItemList(localStorageColumnHeaderItemList);
    ...
    const tempFirstColumnItemListArray = activityColumnHeaderItemList.map((item) => item.columnName);
    ...
    setActivityColumnHeaderItemList(firstColumnItemListArray);
    // isFrozenがtrueの個数をRefに格納
  ...
  const handleMouseDown = (e: React.MouseEvent, index: number) => {
    ...
    const handleMouseUp = () => {
      ...
      const copyColumnHeaderItemList = [...activityColumnHeaderItemList];
  ...
  const columnHeaderLeft = (index: number) => {
    ...
    // isFrozenがtrueなら
    if (activityColumnHeaderItemList[index].isFrozen) {
  ...
  const handleClickGridCell = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      setSelectedRowDataActivity(allRows[Number(selectedGridCellRef.current?.parentElement?.ariaRowIndex) - 2]);
    },
    [allRows]
  );
  ...
  useEffect(() => {
    ...
    const draggingElementColIndex = activityColumnHeaderItemList[dragColumnIndex].columnIndex;
    const dropElementColIndex = activityColumnHeaderItemList[dropIndex!].columnIndex;
    const draggingElementColWidth = activityColumnHeaderItemList[dragColumnIndex].columnWidth;
    const dropElementColWidth = activityColumnHeaderItemList[dropIndex!].columnWidth;
    ...
    console.log("🌟更新前 activityColumnHeaderItemList全体", activityColumnHeaderItemList);
    //  🌟順番を入れ替える activityColumnHeaderItemList
    const copyListItems: ColumnHeaderItemList[] = JSON.parse(JSON.stringify(activityColumnHeaderItemList)); // 一意性を守るため新たなカラムリストを生成
    ...
    // ================= 🔥🔥テスト🔥🔥==================
    setActivityColumnHeaderItemList([...newListItemArray]);
  ...
  // ================= 🔥🔥テスト🔥🔥==================
  const columnOrder = [...activityColumnHeaderItemList].map((item, index) => item.columnName as keyof Client_company); // columnNameのみの配列を取得
  ...
  return (
    <>
          ...
          <div className={`${styles.grid_function_header}`}>
            ...
            <div className={`flex max-h-[26px] w-full min-w-max items-center justify-end space-x-[6px]`}>
              {activeCell?.role === "columnheader" && Number(activeCell?.ariaColIndex) !== 1 && (
                <>
                  <button
                    className={`flex-center transition-bg03 func_btn_green h-[25px] max-h-[25px] min-h-[25px] space-x-2 rounded-[4px] px-[12px] text-[12px]`}
                    onClick={async () => {
                      handleCloseTooltip();
                    }}
                    onMouseEnter={(e) => {
              ...
              {activeCell?.role !== "columnheader" && selectedRowDataActivity && (
                <>
                  {!isLoadingDelete && (
                    <button
                      className={`flex-center transition-bg03 func_btn_red h-[25px] max-h-[25px] min-h-[25px] space-x-2 rounded-[4px] px-[12px] text-[12px]`}
                      onClick={async () => {
                        handleCloseTooltip();
                        ...
                        >
                          <div className={styles.grid_select_cell_header}>
                            <input
                              ...
                              onChange={(e) => {
                                if (typeof rowData?.activity_id === "undefined") return;
                                if (rowData?.activity_id === null) return;
                                console.log(
                                  `クリック VirtualRow.index: ${virtualRow.index} row.activity_id${rowData.activity_id}`
                                );
      ...
      {isOpenEditColumns && <EditColumnsModalDisplayOnly columnHeaderItemList={activityColumnHeaderItemList} />}
      {/* ================== 🌟カラム編集モーダル🌟 ここまで ================== */}
      {/* ================== メインコンテナ ここまで ================== */}
      ...
      {isOpenDeleteModal &&
        ...
        selectedRowDataActivity.activity_created_by_company_id === userProfileState?.company_id && (
          <ConfirmationModal
            ...
            sectionP1={`確定することでこの活動データは完全に削除されます。\nこの操作は確定後、取り消すことができません。`}
            cancelText="戻る"
            submitText="削除を確定"
            buttonColor="red"
            zIndex="3000px"
            ...
            clickEventSubmit={async () => {
              ...
              try {
                ...
                console.log("🔥削除実行 activityId", activityId, "selectedRowDataActivity", selectedRowDataActivity);
...
export const ActivityGridTableAll = memo(ActivityGridTableAllMemo);
