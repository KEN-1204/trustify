...
const EditModalTaskCardMemo = ({ setIsOpenEditModal }: Props) => {
  const editedTaskCard = useDashboardStore((state) => state.editedTaskCard);
  const setEditedTaskCard = useDashboardStore((state) => state.setEditedTaskCard);

  if (!editedTaskCard) return null;

  // 閉じる
  const handleClose = () => {
    setEditedTaskCard(null);
    setIsOpenEditModal(false);
  };

  console.log("EditModalTaskCardレンダリング editedTaskCard", editedTaskCard);
  return (
    <>
        ...
        <div className={`${styles.container}`}>
          <div className={`${styles.title_area} h-[90px] bg-[green]/[0]`}>
            ...
            <div className={`${styles.contents_area} min-w-[600px] bg-[aqua]/[0]`}>
              <h2 className={`${styles.title} text-[24px] font-bold`}>{editedTaskCard.taskTitle}</h2>
              <div className="flex items-center space-x-[6px] text-[13px] text-[var(--color-text-sub)]">
                <span>リスト:</span>
                <span className="underline">{editedTaskCard.columnTitle}</span>
              </div>
            </div>
          </div>
          ...
          <div className={`${styles.description_area} h-[120px] bg-[aqua]/[0]`}>
            <div className={`${styles.title_area} h-full bg-[green]/[0]`}>
              ...
              <div className={`${styles.contents_area} min-w-[450px] bg-[aqua]/[0]`}>
                <h2 className={`${styles.section_title}`}>{editedTaskCard.taskTitle}</h2>
...
export const EditModalTaskCard = memo(EditModalTaskCardMemo);
