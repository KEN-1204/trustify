...
const QuotationGridTableAllMemo: FC<Props> = ({ title }) => {
  ...
  const isUpdateRequiredForLatestSelectedRowDataQuotation = useDashboardStore(
    (state) => state.isUpdateRequiredForLatestSelectedRowDataQuotation
  );
  ...
  // const [quotationColumnHeaderItemList, setQuotationColumnHeaderItemList] = useState<ColumnHeaderItemList[]>([]);
  ...
  useEffect(() => {
    ...
    setQuotationColumnHeaderItemList([...editedColumnHeaderItemList]);
    // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ================
    const quotationColumnHeaderItemListJSON = JSON.stringify(editedColumnHeaderItemList);
    localStorage.setItem("grid_columns_quotations", quotationColumnHeaderItemListJSON);
    // localStorage.setItem("grid_columns_quotation", quotationColumnHeaderItemListJSON);
  ...
  if (userProfileState?.company_id === null) {
    fetchNewSearchServerPage = async (
      ...
      const { data, error, count } = await supabase
        ...
        // .order("quotation_created_at", { ascending: false })
  ...
  if (userProfileState?.company_id) {
    fetchNewSearchServerPage = async (
      ...
      // const { data, error, count } = await supabase
      //   .rpc("search_quotations_and_companies_and_contacts", { params }, { count: "exact" })
      //   // .or(`quotation_created_by_company_id.eq.${userProfileState.company_id},quotation_created_by_company_id.is.null`)
      //   // .or(`quotation_created_by_user_id.eq.${userProfileState.id},quotation_created_by_user_id.is.null`)
      //   .eq("quotation_created_by_company_id", userProfileState.company_id)
      //   // .or(`quotation_created_by_user_id.eq.${userProfileState.id},quotation_created_by_user_id.is.null`)
      //   .range(from, to)
      ...
      //   // .order("quotation_created_at", { ascending: false });
      //   .order("expected_order_date", { ascending: false }) //面談・訪問日(予定)
      //   // .order("quotation_date", { ascending: false }) //面談・訪問日(予定)
      //   .order("quotation_created_at", { ascending: false }); //面談作成日時
      // // 成功バージョン
      // // const { data, error, count } = await supabase
      // //   .rpc("search_quotations_and_companies_and_contacts", { params }, { count: "exact" })
  ...
  // ================= 🔥🔥テスト🔥🔥==================
  // 新規サーチで検索した条件のnewSearchQuotation_Contact_CompanyParamsのオブジェクトを全てa-zで並べ替えた状態で文字列にすることで、
  ...
  const { status, data, error, isFetching, isFetchingNextPage, fetchNextPage, hasNextPage, refetch } = useInfiniteQuery(
    {
      ...
      queryFn: async (ctx) => {
        ...
        // 新規サーチなしの通常モード
        if (newSearchQuotation_Contact_CompanyParams === null) {
          console.log("通常フェッチ queryFn✅✅✅", newSearchQuotation_Contact_CompanyParams);
          return fetchServerPage(50, ctx.pageParam); // 50個ずつ取得
        } else {
          console.log("サーチフェッチ queryFn✅✅✅", newSearchQuotation_Contact_CompanyParams);
  ...
  useEffect(() => {
    ...
    // ========================= 🔥テスト ローカルストレージ ルート =========================
    const localStorageColumnHeaderItemListJSON = localStorage.getItem("grid_columns_quotations");
    ...
    if (localStorageColumnHeaderItemListJSON) {
      ...
      setQuotationColumnHeaderItemList(localStorageColumnHeaderItemList);
    ...
    const newColsWidths = new Array(quotationColumnHeaderItemList.length + 1).fill("120px");
    ...
    const tempFirstColumnItemListArray = quotationColumnHeaderItemList.map((item) => item.columnName);
    ...
    setQuotationColumnHeaderItemList(firstColumnItemListArray);
    // isFrozenがtrueの個数をRefに格納
    isFrozenCountRef.current = firstColumnItemListArray.filter((obj) => obj.isFrozen === true).length;

    // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ================
    const quotationColumnHeaderItemListJSON = JSON.stringify(firstColumnItemListArray);
    localStorage.setItem("grid_columns_quotations", quotationColumnHeaderItemListJSON);
  ...
  const handleMouseDown = (e: React.MouseEvent, index: number) => {
    ...
    const handleMouseUp = () => {
      ...
      // ================ quotationColumnHeaderItemListも合わせてサイズを更新 テスト ================
      let newColumnHeaderItemList: any[] = [];
      const copyColumnHeaderItemList = [...quotationColumnHeaderItemList];
      ...
      setQuotationColumnHeaderItemList(newColumnHeaderItemList);
      // ================ quotationColumnHeaderItemListも合わせてサイズを更新 テスト ================
      ...
      // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ================
      const quotationColumnHeaderItemListJSON = JSON.stringify(newColumnHeaderItemList);
      localStorage.setItem("grid_columns_quotations", quotationColumnHeaderItemListJSON);
      // localStorage.setItem("grid_columns_contacts", contactColumnHeaderItemListJSON);
      // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ここまで ================
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // ===================================================== 🔥テスト フローズンカスタムプロパティ
      const filteredIsFrozenList = quotationColumnHeaderItemList.filter((item) => item.isFrozen === true);
      filteredIsFrozenList.forEach((item, index) => {
  ...
  const columnHeaderLeft = (index: number) => {
    ...
    // isFrozenがtrueなら
    if (quotationColumnHeaderItemList[index].isFrozen) {
  ...
  useEffect(() => {
    ...
    const draggingElementColIndex = quotationColumnHeaderItemList[dragColumnIndex].columnIndex;
    const dropElementColIndex = quotationColumnHeaderItemList[dropIndex!].columnIndex;
    const draggingElementColWidth = quotationColumnHeaderItemList[dragColumnIndex].columnWidth;
    const dropElementColWidth = quotationColumnHeaderItemList[dropIndex!].columnWidth;
    ...
    console.log("🌟更新前 quotationColumnHeaderItemList全体", quotationColumnHeaderItemList);
    //  🌟順番を入れ替える quotationColumnHeaderItemList
    const copyListItems: ColumnHeaderItemList[] = JSON.parse(JSON.stringify(quotationColumnHeaderItemList)); // 一意性を守るため新たなカラムリストを生成
    ...
    // ================= 🔥🔥テスト🔥🔥==================
    setQuotationColumnHeaderItemList([...newListItemArray]);
  ...
  const handleDragEnd = (e: React.DragEvent<HTMLDivElement>) => {
    console.log("Drop✅");
    // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ================
    const quotationColumnHeaderItemListJSON = JSON.stringify(quotationColumnHeaderItemList);
    localStorage.setItem("grid_columns_quotations", quotationColumnHeaderItemListJSON);
    // localStorage.setItem("grid_columns_contacts", contactColumnHeaderItemListJSON);
    // ================ ✅ローカルストレージにも更新後のカラムリストを保存 ここまで ================
    // =============== フローズン用 各カラムのLeft位置、レフトポジションを取得 ===============
  ...
  // ======= 🌟案件編集モーダルでUPDATE後に選択中のセルをクリックさせてselectedRowDataQuotationを最新に更新する🌟
  // UPDATEクエリでDB更新後にinvalidateQueries()でキャッシュは更新するがZustandは更新できていないため、UPDATEクエリ成功時にisUpdateRequiredForLatestSelectedRowDataQuotationをtrueにして発火通知をすることで、選択中のセルを再度クリックさせてselectedRowDataQuotationを再度更新する
  useEffect(() => {
    if (!isUpdateRequiredForLatestSelectedRowDataQuotation) return;
    ...
    setIsUpdateRequiredForLatestSelectedRowDataQuotation(false);
  }, [isUpdateRequiredForLatestSelectedRowDataQuotation, setIsUpdateRequiredForLatestSelectedRowDataQuotation]);
  ...
  // ================= 🔥🔥テスト🔥🔥==================
  const columnOrder = [...quotationColumnHeaderItemList].map((item, index) => item.columnName as keyof Client_company); // columnNameのみの配列を取得
  ...
  const formatDateMapping: {
    ...
    quotation_created_at: "yyyy/MM/dd HH:mm:ss",
    quotation_updated_at: "yyyy/MM/dd HH:mm:ss",
  };
  ...
  return (
    <>
          ...
          <div className={`${styles.grid_function_header}`}>
            <div className={`flex max-h-[26px] w-full items-center justify-start space-x-[6px]`}>
              ...
              <button
                ...
                // }`}
                className={`flex-center transition-bg03 relative h-[25px] max-h-[25px] min-h-[25px] min-w-[118px] space-x-1  rounded-[4px] px-[15px] text-[12px] ${
                  data?.pages[0]?.rows
                ...
                onClick={async () => {
                  ...
                  await queryClient.invalidateQueries({ queryKey: ["quotations"] });
                  // 再度テーブルの選択セルのDOMをクリックしてselectedRowDataQuotationを最新状態にする
            ...
            <div className={`flex max-h-[26px] w-full min-w-max items-center justify-end space-x-[6px]`}>
              ...
              {activeCell?.role !== "columnheader" && selectedRowDataQuotation && (
                <>
                  {!isLoadingDelete && (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (!userProfileState) return alert("ユーザーデータが見つかりませんでした。エラー：QGTA020");
                        if (!userProfileState.account_company_role)
                          return alert("ユーザーデータが見つかりませんでした。エラー：QGTA021");
                        // 自分が作成した行か確認 or 自分以外の行を削除できるのはマネージャークラス以上
                        if (selectedRowDataQuotation.quotation_created_by_user_id !== userProfileState.id) {
                          if (
              ...
              <RippleButton
                ...
                classText="select-none"
                clickEventHandler={() => {
                  const newColumnHeaderItemListReset = JSON.parse(JSON.stringify(quotationColumnHeaderItemList));
                  console.log(
          ...
          >
            ...
            >
...

              {
                ...
                // !!quotationColumnHeaderItemList.length &&
                //   quotationColumnHeaderItemList
                !!quotationColumnHeaderItemList.length &&
                  [...quotationColumnHeaderItemList]
                    ...
                    .map((key, index) => (
                      <div
                        ...
                        data-text={`${columnNameToJapaneseQuotation(key.columnName)}`}
                          ...
                          >
                            <div className={`${styles.grid_column_header_inner} pointer-events-none`}>
                              ...
                              >
                                ...
                                {language === "ja" && columnNameToJapaneseQuotation(key.columnName)}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        >
                          <div className={styles.grid_select_cell_header}>
                            <input
                              ...
                              onChange={(e) => {
                                if (typeof rowData?.quotation_id === "undefined") return;
                                if (rowData?.quotation_id === null) return;
                                console.log(
                                  `クリック VirtualRow.index: ${virtualRow.index} row.quotation_id${rowData.quotation_id}`
                                );
                        ...
                        {rowData ? (
                          ...
                          columnOrder ? (
                            columnOrder
                              ...
                              .map((value, index) => {
                                const columnName = quotationColumnHeaderItemList[index]?.columnName;
                                ...
                                return (
                                  <div
                                    ...
                                    role="gridcell"
                                    aria-colindex={
                                      quotationColumnHeaderItemList[index]
                                        ? quotationColumnHeaderItemList[index]?.columnIndex
                                        : index + 2
                                    } // カラムヘッダーの列StateのcolumnIndexと一致させる
                                    aria-selected={false}
                                    tabIndex={-1}
                                    className={`${styles.grid_cell} ${
                                      quotationColumnHeaderItemList[index].isFrozen ? styles.grid_column_frozen : ""
                                    } ${
                                      isFrozenCountRef.current === 1 && index === 0 ? styles.grid_cell_frozen_last : ""
                                    } ${isFrozenCountRef.current === index + 1 ? styles.grid_cell_frozen_last : ""}  ${
                                    ...
                                    style={
                                      quotationColumnHeaderItemList[index].isFrozen
                                        ? {
                                            gridColumnStart: quotationColumnHeaderItemList[index]
                                              ? quotationColumnHeaderItemList[index]?.columnIndex
                                              : index + 2,
                                    ...
                                    onDoubleClick={(e) =>
                                      handleDoubleClick(e, index, quotationColumnHeaderItemList[index].columnName)
                                    }
                            ...
                            //   >
                            //     {/* {value} */}
                            //     {quotationColumnHeaderItemList[index].columnName === "quotation_date" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd")}
                            //     {quotationColumnHeaderItemList[index].columnName === "scheduled_follow_up_date" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd")}
                            //     {quotationColumnHeaderItemList[index].columnName === "quotation_created_at" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd HH:mm:ss")}
                            //     {quotationColumnHeaderItemList[index].columnName === "quotation_updated_at" &&
                            //       value &&
                            //       format(new Date(value), "yyyy-MM-dd HH:mm:ss")}
                            //     {quotationColumnHeaderItemList[index].columnName !== "scheduled_follow_up_date" &&
                            //       quotationColumnHeaderItemList[index].columnName !== "quotation_date" &&
      ...
      {isOpenEditColumns && <EditColumnsModalDisplayOnly columnHeaderItemList={quotationColumnHeaderItemList} />}
      {/* ================== 🌟カラム編集モーダル🌟 ここまで ================== */}
      {/* ================== メインコンテナ ここまで ================== */}
      ...
      {isOpenDeleteModal &&
        selectedRowDataQuotation &&
        userProfileState &&
        selectedRowDataQuotation.quotation_created_by_company_id === userProfileState?.company_id && (
          <ConfirmationModal
            titleText={`削除してもよろしいですか？`}
            sectionP1={`確定することでこの見積データは完全に削除されます。\nこの操作は確定後、取り消すことができません。`}
            cancelText="戻る"
            submitText="削除を確定"
            buttonColor="red"
            zIndex="3000px"
            ...
            clickEventSubmit={async () => {
              ...
              try {
                const quotationId = selectedRowDataQuotation.quotation_id;
                ...
                console.error("削除エラー： QGTA022", error);
...
export const QuotationGridTableAll = memo(QuotationGridTableAllMemo);
