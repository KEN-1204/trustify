...
export const normalizeCompanyName = (input: string) => {
  ...
  // ・\u005F: 半角アンダースコア（_） - 特に技術関連の企業や製品名に使われることがあります(「＿」：\u005fは半角に変換済み)
  // ・\uFF08: （ 全角括弧
  // ・\uFF09: ） 全角括弧
  // ・「(」（左半角括弧）: \u0028
  // ・「)」（右半角括弧）: \u0029
...
// 🔸人物名の標準化・データクレンジング・バリデーションチェック

export function normalizeRepresentativeName(name: string) {
  // 【下記の指定した文字のみ許可 それ以外は空文字にリプレイス [^...]】
  ...
  let halfName = name
    .replace(/[ａ-ｚＡ-Ｚ]/g, (ch) => String.fromCharCode(ch.charCodeAt(0) - 0xfee0)) // 全角英字を半角に変換
...
// ----------------------------------- 🔸半角カタカナ => 半角カタカナ🔸 -----------------------------------
export function convertKanaHalfWidthToFullWidth(kanaText: string) {
...
export const convertKanaZenkakuToHankakuMap: { [key: string]: string } = {
  ガ: "ｶﾞ",
  ギ: "ｷﾞ",
  グ: "ｸﾞ",
  ゲ: "ｹﾞ",
...
export const zenkakuToHankakuJa = (input: string) => {
  return input.replace(/[ァ-ヶ。、ー「」・]/g, function (str) {
    const reg = new RegExp("(" + Object.keys(convertKanaZenkakuToHankakuMap).join("|") + ")", "g");
    return str
      .replace(reg, function (match: string) {
        return convertKanaZenkakuToHankakuMap[match];
      })
      .replace(/゛/g, "ﾞ")
      .replace(/゜/g, "ﾟ");
  });
