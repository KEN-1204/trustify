...
export const getFiscalQuarterTest = (fiscalYearEnd: DateInput, date: DateInput): number => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    // if (month < 0) {
    //   month += 12;
    //   console.log("monthが0未満のため12を加算", year, month, day);
    // }
    ...
    // ユーザーの決算月がその月の最終日なら、全ての四半期を翌月にして、1日をセットする
    if (isLastDayUserFiscalEnd) {
      // 決算月が末日でかつ12月が決算月の場合、12月のみ既に日付に+1で年と月が翌年に変わっているため、月の加算は不要
      if (end.getMonth() === 11 && month === 11) {
        ...
        // }
        quarterStartDate = new Date(year + yearAdjustment, end.getMonth() - 3 * i, end.getDate() + 1);
        console.log("🌟決算月が12月31日のため別途アップデート不要", format(quarterStartDate, "yyyy年MM月dd日"));
      }
      // 決算月が末日で、12月が決算月でない場合のルート
      else {
        // 2月28日が決算月日の時の処理
        if (day === 28) {
          quarterStartDate = new Date(year + yearAdjustment, end.getMonth() - 3 * i + 1, 1);
          console.log("🌟2月28日が決算月の処理", format(quarterStartDate, "yyyy年MM月dd日"));
          if (checkDate.getMonth() === 1 && checkDate.getDate() === 29) isLeapYear = true;
          console.log("🌟🌟🌟🌟🌟🌟閏年 2月29日");
        } else {
          // quarterStartDate = new Date(year + yearAdjustment, end.getMonth() - 3 * i, end.getDate() + 1);
          // １ヶ月が30日間の月は月にプラス１せず「西向く士 小の月」30
          if ([2, 4, 6, 9, 11].includes(end.getMonth() + 1)) {
            quarterStartDate = new Date(year + yearAdjustment, end.getMonth() - 3 * i + 1, 1);
            console.log("🌟小の月", month + 1);
          } else {
            // １ヶ月が31日間ある月
            quarterStartDate = new Date(year + yearAdjustment, end.getMonth() - 3 * i + 1, 1);
            console.log("🌟大の月", month + 1);
          }
          // quarterStartDate.setMonth(quarterEndDate.getMonth() + 1);
          // quarterStartDate.setDate(1);
          console.log("🌟決算月が末日", format(quarterStartDate, "yyyy年MM月dd日"));
        }
      }
    ...
    console.log(
      `✅forループ ${i}回目`,
      // `💡プラス1していない四半期終了日${format(quarterEndDate, "yyyy年MM月dd日")}`,
      `💡四半期開始日${format(quarterStartDate, "yyyy年MM月dd日")}`
    );

    // quarterStartDate.setHours(0, 0, 0, 0); //開始日の時刻を0に更新
    quarterStartDates.unshift(quarterStartDate); // 最新の四半期から配列に追加(先頭に追加)
    // quarterStartDates.push(quarterStartDate); // 最新の四半期から配列に追加(先頭に追加)
  }
  ...
  for (let i = 0; i < quarterStartDates.length; i++) {
    ...
    // 2月28日が決算月で、かつ2月29日が来た場合は４の第四四半期を返す
    if (isLeapYear) {
      return 4;
    }
