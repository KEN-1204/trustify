export function convertToJapaneseCurrencyFormat(number: number): string {
  // 兆、億、万の単位で分割
  const trillion = Math.floor(number / 1000000); // 1兆 = 1,000,000万
  const billion = Math.floor((number % 1000000) / 10000); // 1億 = 10,000万
  ...
  if (million > 0) {
    formattedString += `${million.toLocaleString()}万円`;
  }
